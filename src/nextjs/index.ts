...
export default async function run(data: initial_input) {
    ...
    execSync(`${command} ${projectName} ${useTypeScript ? "--ts" : "--js"} ${tailwind ? "--tailwind" : "--no-tailwind"} ${eslint ? "--eslint" : "--no-eslint"} ${app ? "--app" : "--no-app"} ${srcDir ? "--src-dir" : "--no-src-dir"} ${`--import-alias ${importAlias}`}`, { stdio: 'inherit' });
}